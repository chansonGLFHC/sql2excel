...
def getconfig():
    ...
    config["WebHookError"].fillna("", inplace = True)
    ...
    try:        
        ...
        for job in config.index:
            ...
            if int(JobID)==int(args.job):
                ...
                JobWebHookError=config['WebHookError'][job]
                ...
                if JobActive==1:
                    ...
                    SendTeamsMessage(JobWebHookSuccess,
                                     "sql2excel "+str(JobOutputName)+" completed", 
                                     "Job "+JobID+" completed successfully")
                    logging.info("Job %s Completed Successfully", str(JobID))
    except Exception as e:
        print("Exception in run runexcel2sql", e)
        traceback.print_exc()
        errmsg = "Job "+str(JobID)+" failed with errors in ["+runexcel2sql+"]. Log File: "+LOG_NAME+"<br>\nMXLoutput: "+traceback.format_exc()
        logging.error("************************************************************************************************" )
        logging.error("***** Job %s Failed",str(JobID))
...
def main():
    try:
        ...
        print("Exception in run main", e)
        traceback.print_exc()
        errmsg = "Job "+str(args.job)+" failed with errors in [main]. Log File: "+LOG_NAME+"<br>\nMXLoutput: "+traceback.format_exc()
        logging.error("************************************************************************************************" )
